...
struct thread_map *thread_map__new_by_uid(uid_t uid)
{
	...
	while ((dirent = readdir(proc)) != NULL) {
		...
		if (grow) {
			struct thread_map *tmp = thread_map__realloc(threads, max_threads);
			if (!tmp)
		...
		for (i = 0; i < items; i++)
			thread_map__set_pid(threads, threads->nr + i, atoi(namelist[i]->d_name));
