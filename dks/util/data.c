...
static bool is_pipe(struct perf_data_file *file)
{
	struct stat st;
	bool is_pipe = false;
	int fd = perf_data_file__is_read(file) ? STDIN_FILENO : STDOUT_FILENO;
...
static int open_file_read(struct perf_data_file *file)
{
	...
	if (!file->force && st.st_uid && (st.st_uid != geteuid()) && geteuid() != 0) {
...
int perf_data_file__open(struct perf_data_file *file)
{
	...
	if (!file->path)
		file->path = DKS_PROFILE_DATA_FILENAME;
