...
void (*spin_lock_func)(volatile int *lock) = NULL;
...
static void spin_ts_lock(volatile int *lock)
{
	atomic_inc(n_waiters);
	while(smp_swap(lock, 1)) {
	};
	atomic_dec(n_waiters);
}

static void spin_tts_lock(volatile int *lock)
{
	while(1) {
...
static void spin_pthread_lock(volatile int *lock)
{
	int ret;
