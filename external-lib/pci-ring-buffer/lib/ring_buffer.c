...
#else
	/* TODO:
	 * do we need to distinguish vmalloc() and vmalloc_user()? */
	rb_addr = vmalloc_user(mmap_size);
			  ...
			  struct ring_buffer_t **prb)
{
	...
	min_size = 2 * (align + sizeof(struct ring_buffer_elm_t));
	if (size_hint < min_size)
		size_hint = min_size;
	rb_size = (size_hint + ~PAGE_MASK) & PAGE_MASK;
...
void rb_wmb(struct ring_buffer_t *rb, int op_type)
{
	...
	if (op_type == RING_BUFFER_OP_GET)
		...
		volatile size_t __dummy; /* it should be in the stack */
		/* enforce previous on-the-fly write completed */
		__dummy = rb_org->head;
		smp_cmb();
		rb_org->head  = rb->head;
		(void)__dummy; /* make a compiler happy */
	}
}
...
size_t ring_buffer_free_space(struct ring_buffer_t *rb, int level)
{
	/* calculate free space more accurately by reaping garbage elements */
	if (level > 0) {
...
int ring_buffer_elm_valid(struct ring_buffer_t *rb __attribute__((unused)), void *data)
{
	static const unsigned short exit_bits =
		RING_BUFFER_ELM_STATUS_INIT |
		RING_BUFFER_ELM_STATUS_READY;
	static const unsigned short non_exit_bits =
		RING_BUFFER_ELM_STATUS_TOMBSTONE |
		RING_BUFFER_ELM_STATUS_TOMBSTONE_OWNER |
		RING_BUFFER_ELM_STATUS_DONE;
	struct ring_buffer_elm_t *elm;
	...
	if ( ((elm_status & exit_bits) == exit_bits) &&
	     ((elm_status & non_exit_bits) == non_exit_bits) ) {
		return 1;
	}
	return 0;
}
EXPORT_SYMBOL(ring_buffer_elm_valid);
