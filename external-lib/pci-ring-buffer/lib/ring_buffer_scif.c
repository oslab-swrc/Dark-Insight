...
void *__wait_for_shadow(void *p)
{
	...
	shadow_type = (rbs->scif[0].type == RING_BUFFER_PRODUCER) ?
...
{
	/* sanity check */
	if ( unlikely(rbs->scif[0].type != RING_BUFFER_PRODUCER) )
		return -EOPNOTSUPP;
...
{
	/* sanity check */
	if ( unlikely(rbs->scif[0].type != RING_BUFFER_CONSUMER) )
		return -EOPNOTSUPP;
...
int ring_buffer_scif_elm_valid(struct ring_buffer_scif_t *rbs,
			       void *data)
{
	return ring_buffer_elm_valid(rbs->rb, data);
}
EXPORT_SYMBOL(ring_buffer_scif_elm_valid);
...
size_t ring_buffer_scif_free_space(struct ring_buffer_scif_t *rbs, int level)
{
	/* NOTE: only a producer can reap elements */
	if (level && rbs->scif[0].type != RING_BUFFER_PRODUCER) {
		level = 0;
	}
