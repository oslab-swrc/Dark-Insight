...
#include "subcmd/subcmd-util.h"
#include "subcmd/parse-options.h"
#include "subcmd/subcmd-config.h"
...
match:
	...
	return abbrev_option ? get_value(p, abbrev_option, abbrev_flags) : -2;
}
				...
				int argc, const char **argv, int flags)
{
	...
	ctx->opt = NULL;
	if ((flags & PARSE_OPT_KEEP_UNKNOWN) && (flags & PARSE_OPT_STOP_AT_NON_OPTION))
...
int parse_options(int argc, const char **argv, const struct option *options,
	const char * const subcommands[], const char *usage_str[], int flags)
{
	struct parse_opt_ctx_t ctx;

	/* build usage string if it's not provided */
	if (subcommands && !usage_str[0]) {
		char *buf = NULL;
		int i;
	...
	switch (parse_options_step(&ctx, options, usage_str)) {
	case PARSE_OPT_HELP:
		exit(129);
	case PARSE_OPT_DONE:
		break;
	...
	default:
		if (ctx.argv[0][1] == '-')
			astrcatf(&error_buf, "unknown option `%s'",  ctx.argv[0] + 2);
		else
