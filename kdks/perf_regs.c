...
/*This is not export_symbol,
  so copied it from kernel source*/
u64 perf_regs__get_reg_value(struct pt_regs *regs, int idx)
{
	const struct regname_offset *roff;
	unsigned long val = 0;
	union x86_64_reg ret;

	if (WARN_ON_ONCE(idx >= ARRAY_SIZE(regname_to_perf_reg)))
		return KDKS_INVALID_ADDR;

	roff = &regname_to_perf_reg[idx];
	val = regs_get_register(regs, pt_regs_offset[roff->perf_offset]);
	if(!val)
		return KDKS_INVALID_ADDR; 

	ret.q = (u64) val;
	/*get width*/
	switch(roff->reg_width){
		case X86_REG_QWORD:
			return ret.q;
		case X86_REG_DWORD:
			return (u64)ret.d;
...
u16 perf_regs__regname_to_offset(const char* perf_reg) {
	u16 idx = 0; /*index of regname mapping table*/
	const struct regname_offset *roff;
...
const char *perf_regs__get_reg_name(u16 idx){
	if (WARN_ON_ONCE(idx >= ARRAY_SIZE(regname_to_perf_reg)))
		return "NULL";
	return regname_to_perf_reg[idx].name;
}
